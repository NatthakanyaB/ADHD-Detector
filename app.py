...
def main():
    ...
    # Make predictions for each image if available
    if imageaxial is not None:
        image_tensoraxial = preprocess_image(imageaxial)
        pred_axial, _, _ = modelaxial.predict(image_tensoraxial)
        predictions.append(pred_axial)

    if imagecoronal is not None:
        image_tensorcoronal = preprocess_image(imagecoronal)
        pred_coronal, _, _ = modelcoronal.predict(image_tensorcoronal)
        predictions.append(pred_coronal)

    if imagesagittal is not None:
        image_tensorsagittal = preprocess_image(imagesagittal)
        pred_sagittal, _, _ = modelsagittal.predict(image_tensorsagittal)
        predictions.append(pred_sagittal)

    # Display individual predictions
    if predictions:
        for i, pred in enumerate(predictions):
            st.subheader(f"Prediction from Model {i + 1}:")
            st.write(f"Predicted label: {pred}")

        # Perform hard voting using if-else
        ...
        if adhd_count > normal_count:
            ...
            final_prediction = "Undecided"  # In case of a tie

        st.subheader("Final Prediction:")
        st.write(f"The final predicted label based on hard voting is: {final_prediction}")
